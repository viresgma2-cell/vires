...
const nextCandleBtn = document.getElementById('nextCandleBtn');
...
function updateMetrics() {
  ...
  const data = [
    ...
    ['현재 봉 인덱스', `${state.currentIndex + 1}/${state.candles.length}`],
  ];
...
function executePendingAtOpen(openPrice) {
  ...
  if (buyMap[action]) {
    ...
    if (qty > 0) {
      ...
      state.cash -= cost + fee;
      state.avgPrice = state.qty + qty > 0
        ? ((state.avgPrice * state.qty) + cost + fee) / (state.qty + qty)
        : 0;
      state.qty += qty;
      state.buyCount += 1;
...
async function loadData() {
  try {
    ...
    const limit = Math.max(100, Math.min(1000, Number(limitInput.value) || 400));
    const startVal = startInput.value ? new Date(startInput.value).getTime() : Date.now() - (limit * 60_000);
    ...
    renderChart();
    updateMetrics();
    statusEl.textContent = `다운로드 완료: ${symbol} ${state.candles.length}개 봉`;
...
nextCandleBtn.addEventListener('click', nextCandle);
