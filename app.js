...
function createCandleSeries(targetChart) {
  if (typeof targetChart.addCandlestickSeries === 'function') {
  ...
  if (window.LightweightCharts?.CandlestickSeries && typeof targetChart.addSeries === 'function') {
    return targetChart.addSeries(window.LightweightCharts.CandlestickSeries, {
      upColor: '#3dc985',
      downColor: '#ef5350',
      borderVisible: false,
      wickUpColor: '#3dc985',
  ...
  throw new Error('캔들 시리즈 생성 API를 찾을 수 없습니다.');
}
...
function queueAction(action) {
  if (!state.candles.length) {
    setStatus('먼저 데이터를 다운로드하거나 랜덤 훈련을 시작하세요.');
    return;
  }
  if (state.ended) {
    setStatus('훈련이 종료되었습니다. 새 데이터를 불러오세요.');
    return;
  }
...
function nextCandle() {
  if (!state.candles.length) {
    setStatus('진행할 데이터가 없습니다. 먼저 데이터를 불러오세요.');
    return;
  }
  if (state.ended) {
    setStatus('훈련이 이미 종료되었습니다.');
    return;
  }
...
async function randomStart() {
  ...
  const loaded = await loadData({ symbol: 'BTCUSDT', limit: 500, startTime: randomStartTime });
  if (loaded && state.candles.length > 0 && !state.ended) {
    setStatus('랜덤 훈련 시작 완료. 다음 봉 진행 가능');
  }
}
